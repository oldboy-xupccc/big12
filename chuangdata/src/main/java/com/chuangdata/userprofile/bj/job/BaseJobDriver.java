...
import com.chuangdata.userprofile.job.decrypt.DecryptJobDriver;
...
import com.chuangdata.userprofile.job.transform.TransformJobDriver ;

public class BaseJobDriver extends JobDriver {

    public BaseJobDriver(String[] args) {
        super(args);
}
    ...
    protected int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException, URISyntaxException {
        ...
        setDistributedCache(args[8], "chuangdata_resource_app_domain");
        ...
        // 3. set Job
        Job job = Job.getInstance(getConfiguration(), BaseJobDriver.class.getSimpleName());
        job.setJarByClass(BaseJobDriver.class);
    ...
    public static void main(String[] args) throws Exception {
        JobDriver jobDriver = new BaseJobDriver(args);
        int result = jobDriver.run();
         if(result==0){
             JobDriver decryptJobDriver = new DecryptJobDriver(args);
            result = decryptJobDriver.run();

             if(result==0){
                 JobDriver transformJobDriver = new TransformJobDriver(args);
                 result = transformJobDriver.run();
             }
        }
        System.exit(result);
    }
