...
public class GDPIJobDriver extends JobDriver {

    public GDPIJobDriver(String[] args) {
        super(args);
    }
    ...
    protected int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException, URISyntaxException {
        ...
        // 3. set Job
        Job job = Job.getInstance(getConfiguration(), GDPIJobDriver.class.getSimpleName());
        job.setJarByClass(GDPIJobDriver.class);
        job.setMapperClass(GDPIMapper.class);
        job.setPartitionerClass(TimePartitioner.class);
        job.setReducerClass(GDPIReducer.class);
    ...
    public static void main(String[] args) throws Exception {
        JobDriver jobDriver = new GDPIJobDriver(args);
