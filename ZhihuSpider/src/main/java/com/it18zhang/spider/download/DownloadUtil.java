...
public class DownloadUtil {
	...
	//用户代理
	private final static String userAgent = Constants.SPIDER_USERAGENT;
	...
	private static void init() {
		try {
			...
			SSLContext sslContext = SSLContexts.custom().loadTrustMaterial(ks, ts).build();

			SSLConnectionSocketFactory sslSF = new SSLConnectionSocketFactory(sslContext);

			// 创建注册表，对不同协议使用不同的socket工厂
			Registry<ConnectionSocketFactory> registry = RegistryBuilder.<ConnectionSocketFactory>create().register(
					"http", PlainConnectionSocketFactory.INSTANCE).register("https", sslSF).build();

			// 连接管理器
			PoolingHttpClientConnectionManager connMgr = new PoolingHttpClientConnectionManager(registry);

			// socket配置
			SocketConfig socketConfig = SocketConfig.custom().setSoTimeout(Constants.SOCKET_TIMEOUT_MS).setTcpNoDelay(
					true).build();
			...
			// HttpClient构建器
			HttpClientBuilder httpClientBuilder = HttpClients.custom().setConnectionManager(connMgr).setRetryHandler(
					retryHandler).setDefaultCookieStore(new BasicCookieStore()).setUserAgent(userAgent);
			// 代理为空
			if (proxy != null) {
	...
	public static CloseableHttpResponse getResponse(HttpRequestBase request) throws IOException {
		...
		request.setHeader("User-Agent",
				Constants.SPIDER_USERAGENTS.get(new Random().nextInt(Constants.SPIDER_USERAGENTS.size())));
...
//		InputStream fis = HttpClientUtil.class.getResourceAsStream(name);
	...
	public static void downloadFile(HttpRequestBase requestBase, String path, String saveFileName, Boolean isReplaceFile) {
		try {
	...
	public static void setCookieStore(CookieStore cookieStore) {
		DownloadUtil.cookieStore = cookieStore;
	}
	...
	public static org.apache.http.client.config.RequestConfig.Builder getRequestConfigBuilder() {
		...
		builder.setSocketTimeout(Constants.SOCKET_TIMEOUT_MS);
		builder.setConnectTimeout(Constants.SOCKET_TIMEOUT_MS);
		builder.setConnectionRequestTimeout(Constants.SOCKET_TIMEOUT_MS);
